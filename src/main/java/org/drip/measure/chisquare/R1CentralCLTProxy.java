 ...
 *
 *  <ul>
 * 		<li><i>R1CentralCLTProxy</i> Constructor</li>
	...
	extends R1Distribution
{
	...
	private R1UnivariateNormal _r1UnivariateNormal = R1UnivariateNormal.Standard();

	/**
	 * <i>R1CentralCLTProxy</i> Constructor
	 * 
		...
		throws Exception
	{
		if (0 >= (_degreesOfFreedom = degreesOfFreedom)) {
			throw new Exception ("R1CentralCLTProxy Constructor => Invalid Inputs");
		}
	}
	...
	{
		return new double[] {-1. * Math.sqrt (0.5 * _degreesOfFreedom), Double.POSITIVE_INFINITY};
	}
		...
		throws Exception
	{
		...
		return (sumOfStandardNormalSquares - _degreesOfFreedom) / Math.sqrt (2. * _degreesOfFreedom);
	}
}
