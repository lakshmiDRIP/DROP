...
public class R1DegenerateMGF
	extends R1MomentGeneratingFunction
{

	/**
	 * <i>R1DegenerateMGF</i> Constructor
	 * 
	 * @param r1DegenerateDistribution Underlying R<sup>1</sup> Degenerate Distribution
	 * 
	 * @throws Exception Thrown if the Inputs are Invalid
	 */

	public R1DegenerateMGF (
		final R1DegenerateDistribution r1DegenerateDistribution)
		throws Exception
	{
		super (r1DegenerateDistribution);
	}
		...
		throws Exception
	{
		if (!NumberUtil.IsValid (t) || 0. > t) {
			throw new Exception ("R1DegenerateMGF::evaluate => t is Invalid");
		}

		return Math.exp (t * ((R1DegenerateDistribution) distribution()).a());
	}
	...
	{
		if (0 >= momentIndex) {
			throw new Exception ("R1DegenerateMGF::nonCentralMoment - Invalid Moment Index");
		}

		return Math.pow (((R1DegenerateDistribution) distribution()).a(), momentIndex);
	}
}
