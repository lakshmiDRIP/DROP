...
/**
 * <i>R1ContinuousUniform</i> implements the Univariate R<sup>1</sup> Uniform, i. e., Lebesgue Distribution.
 * 	It implements the Incremental, the Cumulative, and the Inverse Cumulative Distribution Densities. It
 * 	provides the following Functionality:
 *
 *  <ul>
 * 		<li>Construct a Standard (0, 1) R<sup>1</sup> Univariate Uniform Distribution</li>
 * 		<li><i>R1ContinuousUniform</i> Constructor</li>
	...
	extends R1Continuous
{
	...
	{
		try {
			return new R1ContinuousUniform (0., 1.);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	/**
	 * <i>R1ContinuousUniform</i> Constructor
	 * 
		...
		throws Exception
	{
		...
		{
			throw new Exception ("R1ContinuousUniform Constructor => Invalid Inputs");
		}
	}
	...
	{
		if (!supported (x)) {
			throw new Exception ("R1ContinuousUniform::cumulative => Invalid Inputs");
		}
		...
		throws Exception
	{
		if (!NumberUtil.IsValid (p) || 1. < p || 0. > p) {
			throw new Exception ("R1ContinuousUniform::invCumulative => Cannot calculate");
		}
	...
	{
		if (!supported (x)) {
			throw new Exception ("R1ContinuousUniform::density => Invalid Inputs");
		}
	...
	@Override public Array2D histogram()
	{
		...
		double width = (_rightSupport - _leftSupport) / GRID_WIDTH;

		for (int gridIndex = 0; gridIndex < GRID_WIDTH; ++gridIndex) {
			yArray[gridIndex] = 1. / GRID_WIDTH;
			xArray[gridIndex] = _leftSupport + (gridIndex + 1) * width;
		}
