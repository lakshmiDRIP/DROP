...
/**
 * <i>I1UniformDistribution</i> implements the Univariate Bounded Uniform Integer Distribution, with the
...
public class I1UniformDistribution
	...
	public I1UniformDistribution (
		final int start,
		final int finish)
		throws Exception
	{
		if ((_finish = finish) <= (_start = start)) {
			throw new Exception ("I1UniformDistribution constructor: Invalid inputs");
		}
	}
		...
		throws Exception
	{
		if (!NumberUtil.IsValid (x)) {
			throw new Exception ("I1UniformDistribution::cumulative => Invalid inputs");
		}
		...
		throws Exception
	{
		if (!NumberUtil.IsValid (y) || y < 0. || y > 1.) {
			throw new Exception ("I1UniformDistribution::invCumulative => Invalid inputs");
		}
	...
	@Override public double density (
		...
		throws Exception
	{
		throw new Exception ("I1UniformDistribution::density => Not available for discrete distributions");
	}
