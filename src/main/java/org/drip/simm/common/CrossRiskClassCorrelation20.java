...
public class CrossRiskClassCorrelation20
{
	...
	/**
	 * Generate the Corresponding Risk Class Correlation Matrix as a <code>LabelCorrelation</code> Instance
	 * 
	...
	public static final LabelledVertexCorrelation Matrix()
	{
		...
		chargramList.add (Chargram.IR);

		chargramList.add (Chargram.CRQ);

		chargramList.add (Chargram.CRNQ);

		chargramList.add (Chargram.EQ);

		chargramList.add (Chargram.CT);

		chargramList.add (Chargram.FX);

		try {
			return new LabelledVertexCorrelation (chargramList, riskClassCorrelationMatrix);
		} catch (Exception e) {
			e.printStackTrace();
		}
