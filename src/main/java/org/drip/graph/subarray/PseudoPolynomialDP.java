	...
	extends org.drip.graph.subarray.SubsetSum
{
	...
	@Override public boolean targetSumExists()
	{
		...
		int[] sortedNumberArray = numberArray();
		...
		int arrayLength = sortedNumberArray.length;
		boolean[] subsetTargetSumExistence = new boolean[arrayLength];
		subsetTargetSumExistence[0] = sortedNumberArray[0] == target;
		int sum = sortedNumberArray[0];

		for (int sortedNumber : sortedNumberArray)
		{
			if (0 > sortedNumber)
				sumOfNegativeValues = sumOfNegativeValues + sortedNumber;
			else if (0 < sortedNumber)
				sumOfPositiveValues = sumOfPositiveValues + sortedNumber;
		}
			...
			++index)
		{
			subsetTargetSumExistence[index] = subsetTargetSumExistence[index - 1] ||
				sortedNumberArray[index] == target || subsetSumSet.contains (
					target - sortedNumberArray[index]
				);

			subsetSumSet.add (
				sortedNumberArray[index]
			);

			subsetSumSet.add (
				sum + sortedNumberArray[index]
			);
		}

		return subsetTargetSumExistence[arrayLength - 1];
	}
}
