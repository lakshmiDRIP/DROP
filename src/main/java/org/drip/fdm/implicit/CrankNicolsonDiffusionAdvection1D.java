...
/**
 * <i>CrankNicolsonDiffusionAdvection1D</i> implements the 1D Crank-Nicolson Discretized Diffusion-Advection
 * 	State-Space Evolution Scheme. The References are:
 * 
 * <br><br>
 * 	<ul>
 * 		<li>
...
public class CrankNicolsonDiffusionAdvection1D
{
	private boolean _diagnosticsOn = false;
	private RdToR1 _advectionFunction = null;
	private RdToR1 _diffusionFunction = null;
	private EvolutionGrid1D _evolutionGrid1D = null;

	/**
	 * <i>CrankNicolsonDiffusionAdvection1D</i> Constructor
	 * 
	 * @param evolutionGrid1D R<sup>1</sup> Evolution Increment
	 * @param diffusionFunction Diffusion Function
	 * @param advectionFunction Advection Function
	 * @param diagnosticsOn TRUE - "Diagnostics On" Mode
	 * 
	 * @throws Exception Thrown if the Inputs are Invalid
	 */

	protected CrankNicolsonDiffusionAdvection1D (
		final EvolutionGrid1D evolutionGrid1D,
		final RdToR1 diffusionFunction,
		final RdToR1 advectionFunction,
		final boolean diagnosticsOn)
		throws Exception
	{
		if (null == (_evolutionGrid1D = evolutionGrid1D) ||
			null == (_diffusionFunction = diffusionFunction) ||
			null == (_advectionFunction = advectionFunction))
		{
			throw new Exception ("CrankNicolsonDiffusionAdvection1D Constructor => Invalid Inputs");
		}
	...
	/**
	 * Retrieve the R<sup>d</sup> to R<sup>1</sup> Advection Function
	 * 
	 * @return R<sup>d</sup> to R<sup>1</sup> Advection Function
	 */

	public RdToR1 advectionFunction()
	{
		return _advectionFunction;
	}
