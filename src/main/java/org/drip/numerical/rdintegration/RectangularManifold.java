...
/**
 * <i>RectangularManifold</i> holds a Rectangular Quadrature Manifold corresponding to an Integration of
...
public class RectangularManifold
	extends BoundedManifold
{
	...
	@Override public double[] leftCartesianBoundArray()
	{
	...
	@Override public double[] rightCartesianBoundArray()
	{
	...
	@Override public RectangularManifold[] evenlySplitAcrossDimension (
		final int splitDimensionIndex)
	{
		...
		try {
			return new RectangularManifold[] {
				new RectangularManifold (_leftBoundArray, adjustedRightBoundArray),
				new RectangularManifold (adjustedLeftBoundArray, _rightBoundArray)
			};
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	@Override public boolean in00 (
		final double[] rdPoint)
	{
		if (null == rdPoint || rdPoint.length != _leftBoundArray.length) {
			return false;
		}

		for (int dimensionIndex = 0; dimensionIndex < _leftBoundArray.length; ++dimensionIndex) {
			if (rdPoint[dimensionIndex] <= _leftBoundArray[dimensionIndex] ||
				rdPoint[dimensionIndex] >= _rightBoundArray[dimensionIndex])
			{
				return false;
			}
		}
		...
		int validVariateTrial)
	{
		...
		double[] randomRd = new double[rightCartesianBoundArray.length];
