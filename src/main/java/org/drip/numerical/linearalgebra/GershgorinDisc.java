...
public class GershgorinDisc
{
	...
	private double _diagonalEntry = Double.NaN;
	private boolean _isDiagonallyDominant = false;

	/**
	 * Construct an Instance of <i>GershgorinDisc</i> from the Square Matrix's Row
	 * 
	 * @param row Square Matrix Row
	 * @param diagonalIndex Diagonal Element Index
	 * @param r1ClosenessVerifier <i>R1ClosenessVerifier</i> Instance
	 * 
	 * @return <i>GershgorinDisc</i> Instance
	 */

	public static final GershgorinDisc FromRow (
		final double[] row,
		final int diagonalIndex,
		final R1ClosenessVerifier r1ClosenessVerifier)
	{
		if (null == row || row.length < diagonalIndex || null == r1ClosenessVerifier) {
			return null;
		}
		...
		for (int i = 0; i < row.length; ++i) {
			radius += (diagonalIndex == i ? 0. : Math.abs (row[i]));
		}

		try {
			return new GershgorinDisc (
				row[diagonalIndex],
				radius,
				radius / Math.abs (row[diagonalIndex]) < r1ClosenessVerifier.relativeTolerance() ||
					radius < r1ClosenessVerifier.absoluteTolerance()
			);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	/**
	 * <i>GershgorinDisc</i> Constructor
	 * 
	 * @param diagonalEntry Diagonal Entry
	 * @param radius Radius
	 * @param isDiagonallyDominant TRUE - The Row is Diagonally Dominant
	 * 
	 * @throws Exception Thrown if the Inputs are Invalid
	 */

	public GershgorinDisc (
		final double diagonalEntry,
		final double radius,
		final boolean isDiagonallyDominant)
		throws Exception
	{
		if (!NumberUtil.IsValid (_diagonalEntry = diagonalEntry) || 0. == _diagonalEntry ||
			!NumberUtil.IsValid (_radius = radius) || 0. > _radius)
		{
			throw new Exception ("GershgorinDisc Constructor => Invalid Inputs");
		}

		_isDiagonallyDominant = isDiagonallyDominant;
	}

	/**
	 * Retrieve the Gershgorin Disc Radius
	 * 
	 * @return Gershgorin Disc Radius
	 */
