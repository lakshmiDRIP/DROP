...
/**
 * <i>WindschitlTothAnalyticEstimator</i> implements the Windschitl-Toth Version of Log Gamma Estimator. The
 * 	References are:
 * 
 * <br><br>
 * 	<ul>
 ...
 *
 *  <ul>
 * 		<li><i>WindschitlTothAnalyticEstimator</i> Constructor</li>
 *  </ul>
 *
 *  <br>
 *  <style>table, td, th {
...
public class WindschitlTothAnalyticEstimator extends R1ToR1Estimator
{
	...
	/**
	 * <i>WindschitlTothAnalyticEstimator</i> Constructor
	 * 
	 * @param derivativeControl The Derivative Control
	 */

	public WindschitlTothAnalyticEstimator (
		...
		throws Exception
	{
		if (!NumberUtil.IsValid (x) || 0. > x) {
			throw new Exception ("WindschitlTothAnalyticEstimator::evaluate => Invalid Inputs");
		}

		return 0.5 * Math.log (2. * Math.PI / x) + x * (
			Math.log (x) + 0.5 * Math.log (x * Math.sinh (1. / x) + 1. / (810. * x * x * x * x * x * x)) - 1.
		);
	}
}
