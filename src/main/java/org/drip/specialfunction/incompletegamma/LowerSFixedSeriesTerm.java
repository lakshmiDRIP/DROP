 ...
 *
 *  <ul>
 * 		<li>Construct the Weierstrass Limit Version of the Lower S Fixed Term</li>
 * 		<li>Construct the NIST (2019) Limit Version of the Lower S Fixed Term</li>
 *  </ul>
 *
 *  <br>
 *  <style>table, td, th {
...
public class LowerSFixedSeriesTerm
{
	...
	public static final R1ToR1SeriesTerm WeierstrassLimit (
		final double s)
	{
		try {
			return new R1ToR1SeriesTerm() {
					...
					throws Exception
				{
					...
					return (0 == z ? 1. : Math.pow (z, order)) /
						new NemesAnalytic (null).evaluate (s + order + 1);
				}
			};
		} catch (Exception e) {
			e.printStackTrace();
		...
		final double logGammaS)
	{
		try {
			return new R1ToR1SeriesTerm() {
				...
				{
					if (0 > order || !NumberUtil.IsValid (z)) {
						throw new Exception ("LowerSFixedSeriesTerm::NIST2019::value => Invalid Inputs");
					}

					return (order % 2 == 0 ? 1. : -1.) * Math.exp (
						order * Math.log (z) - Math.log (s + order) - (
							0 == order ? 0. : new NemesAnalyticEstimator (null).evaluate (order)
						) - logGammaS
					);
				}
			};
