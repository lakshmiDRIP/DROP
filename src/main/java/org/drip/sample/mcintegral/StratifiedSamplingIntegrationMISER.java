...
/**
 * <i>StratifiedSamplingIntegrationMISER</i> illustrates Stratified Sampling Monte-Carlo Integration of
 * 	R<sup>d</sup> to R<sup>1</sup> Objective Function using the MISER Sample Allocation Scheme. The
 * 	References are:
 * 
 * <br><br>
 * 	<ul>
...
public class StratifiedSamplingIntegrationMISER
{
		...
		throws Exception
	{
		...
		MonteCarloRunStratifiedDiagnostics stratifiedSamplingRunDiagnostics =
			(MonteCarloRunStratifiedDiagnostics) recursiveStratifiedSamplingIntegrator.quadratureRun();

		System.out.println ("\n\t|-------------------------------------------------|");

		System.out.println ("\t|   3D MONTE CARLO STRATIFIED SAMPLING RUN - CORE   |");

		System.out.println ("\t|-------------------------------------------------|");

		System.out.println (
			"\t| Integrand Mean              => " + FormatUtil.FormatDouble (
				stratifiedSamplingRunDiagnostics.integrandMean(),
				1,
				4,
				1.
			)
		...
		System.out.println (
			"\t| Quadrature Value (Estimate) => " + FormatUtil.FormatDouble (
				stratifiedSamplingRunDiagnostics.quadratureValue(),
				1,
				4,
				1.
			)
		...
		System.out.println (
			"\t| Quadrature Variance         => " + FormatUtil.FormatDouble (
				stratifiedSamplingRunDiagnostics.quadratureVariance(),
				1,
				9,
				1.
			)
		...
		System.out.println (
			"\t| Quadrature Error            => " + FormatUtil.FormatDouble (
				stratifiedSamplingRunDiagnostics.quadratureError(),
				1,
				9,
				1.
			)
		...
		System.out.println (
			"\t| Quadrature Error Percent    => " + FormatUtil.FormatDouble (
				stratifiedSamplingRunDiagnostics.quadratureErrorPercent(),
				1,
				2,
				100.
			) + "%"
		...
		Map<String, MonteCarloRunManifoldDiagnostics> manifoldMap =
			stratifiedSamplingRunDiagnostics.manifoldMap();

		Set<String> setManifoldKey = manifoldMap.keySet();

		System.out.println ("\t| Quadrature Manifold Keys    => " + setManifoldKey);

		System.out.println (
			"\t| Quadrature Zones            => " +
				stratifiedSamplingRunDiagnostics.integrandSampleArrayZoneMap().keySet()
		);

		System.out.println ("\t|-------------------------------------------------|");

		for (String manifoldKey : setManifoldKey) {
			System.out.println ("\n\t|-------------------------------------------------|");

			System.out.println ("\t|" + manifoldMap.get (manifoldKey).toString ("\t"));
