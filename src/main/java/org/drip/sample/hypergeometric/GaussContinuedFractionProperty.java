...
public class GaussContinuedFractionProperty
{
	...
	private static final void Verifier (
		...
		final double c,
		final double[] zArray,
		final R1ToR1Property gaussContinuedFractionProperty)
		throws Exception
	{
		for (double z : zArray)
		{
			R1PropertyVerification propertyVerification = gaussContinuedFractionProperty.verify (z);
		...
		throws Exception
	{
		...
		System.out.println ("\t|             GAUSS CONTINUED FRACTION PROPERTY VERIFIER            ||");
			...
			for (double b : bArray)
			{
				for (double c : cArray)
				{
					Verifier (
						...
						c,
						zArray,
						HypergeometricEqualityLemma.GaussContinuedFractionRecursive (
							a,
							b,
							c
						)
