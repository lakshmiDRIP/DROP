...
/**
 * <i>GaussContiguousProperty3</i> verifies the Gauss Contiguous Identity #3 Lemma for Hyper-geometric
...
public class GaussContiguousProperty3
{
	...
	private static final void Verifier (
		...
		final double c,
		final double[] zArray,
		final R1ToR1Property gaussContiguousProperty3)
		throws Exception
	{
		System.out.println ("\t|----------------------------------------------||");

		System.out.println ("\t|        GAUSS CONTIGUOUS PROPERTY #3          ||");
		...
		for (double z : zArray)
		{
			R1PropertyVerification propertyVerification = gaussContiguousProperty3.verify (z);
			...
			for (double b : bArray)
			{
				for (double c : cArray)
				{
					Verifier (
						...
						c,
						zArray,
						GaussContiguousEqualityLemma.RelationIdentity3 (
							a,
							b,
							c
						)
