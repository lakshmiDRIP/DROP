...
/**
 * <i>GaussContiguousProperty4</i> verifies the Gauss Contiguous Identity #4 Lemma for Hyper-geometric
...
public class GaussContiguousProperty4
{
	...
	private static final void Verifier (
		...
		final double c,
		final double[] zArray,
		final R1ToR1Property gaussContiguousProperty4)
		throws Exception
	{
		System.out.println ("\t|----------------------------------------------||");

		System.out.println ("\t|        GAUSS CONTIGUOUS PROPERTY #4          ||");
		...
		for (double z : zArray)
		{
			R1PropertyVerification propertyVerification = gaussContiguousProperty4.verify (z);
			...
			for (double b : bArray)
			{
				for (double c : cArray)
				{
					Verifier (
						...
						c,
						zArray,
						GaussContiguousEqualityLemma.RelationIdentity4 (
							a,
							b,
							c
						)
