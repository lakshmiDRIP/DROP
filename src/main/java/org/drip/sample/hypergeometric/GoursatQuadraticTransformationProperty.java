...
public class GoursatQuadraticTransformationProperty
{
	...
	private static final void Verifier (
		...
		final double b,
		final double[] zArray,
		final R1ToR1Property goursatQuadraticTransformationProperty)
		throws Exception
	{
		System.out.println ("\t|----------------------------------------------||");

		System.out.println ("\t|  GOURSAT QUADRATIC TRANSFORMATION PROPERTY   ||");
		...
		for (double z : zArray)
		{
			R1PropertyVerification propertyVerification = goursatQuadraticTransformationProperty.verify (z);
		...
		throws Exception
	{
		...
		for (double ab : abArray)
		{
			Verifier (
				ab,
				ab,
				zArray,
				HypergeometricEqualityLemma.GoursatQuadraticTransformation (
					ab,
					ab
				)
			);
