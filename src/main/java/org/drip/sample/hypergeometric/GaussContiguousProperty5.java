...
/**
 * <i>GaussContiguousProperty5</i> verifies the Gauss Contiguous Identity #5 Lemma for Hyper-geometric
...
public class GaussContiguousProperty5
{
	...
	private static final void Verifier (
		...
		final double c,
		final double[] zArray,
		final R1ToR1Property gaussContiguousProperty5)
		throws Exception
	{
		System.out.println ("\t|----------------------------------------------||");

		System.out.println ("\t|        GAUSS CONTIGUOUS PROPERTY #5          ||");
		...
		for (double z : zArray)
		{
			R1PropertyVerification propertyVerification = gaussContiguousProperty5.verify (z);
			...
			for (double b : bArray)
			{
				for (double c : cArray)
				{
					Verifier (
						...
						c,
						zArray,
						GaussContiguousEqualityLemma.RelationIdentity5 (
							a,
							b,
							c
						)
