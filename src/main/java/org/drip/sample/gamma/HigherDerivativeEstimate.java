...
public class HigherDerivativeEstimate
{
		...
		throws Exception
	{
		...
		System.out.println ("\t|                                         HIGHER ORDER GAMMA FUNCTION DERIVATIVE                                         ||");
		...
		for (double z : zArray)
		{
			String display = "\t|" + FormatUtil.FormatDouble (z, 1, 2, 1.) + " => ";

			for (int order : orderArray)
			{
				display = display + FormatUtil.FormatDouble (
					eulerIntegralSecondKind.derivative (
						z,
						order
					), 8, 4, 1.
				) + " |";
