...
/**
 * <i>SemiReplicationZeroThresholdFunding</i> examines the Basel BCBS 2012 OTC Accounting Impact to a
...
public class SemiReplicationZeroThresholdFunding {
		...
		for (int i = 0; i < iNumPath; ++i) {
			...
			for (int j = 0; j <= iNumStep; ++j) {
				...
				if (0 != j) {
					...
					aCGV1[j] = BurgardKjaerBuilder.SemiReplicationDualBond (
						adtVertex[j],
					...
					aCGV2[j] = BurgardKjaerBuilder.SemiReplicationDualBond (
						adtVertex[j],
