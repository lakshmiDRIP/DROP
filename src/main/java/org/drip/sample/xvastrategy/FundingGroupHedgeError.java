...
/**
 * <i>FundingGroupHedgeError</i> demonstrates the Simulation Run of the Funding Group Exposure using the
 * 	"Hedge Error" Funding Strategy laid out in Burgard and Kjaer (2013). The References are:
 *
 *  <br><br>
 *  <ul>
 *  	<li>
...
public class FundingGroupHedgeError {
		...
		throws Exception
	{
		...
		int iNumStep = 10;
		double dblTime = 5.;
		double dblFixedHedgeError = 0.01;
		...
		for (int i = 0; i <= iNumStep; ++i)
		{
			...
			aMV[i] = MarketVertex.Nodal (
				...
				new MarketVertexEntity (
					Math.exp (-0.5 * dblBankHazardRate * (iNumStep - i)),
					dblBankHazardRate,
					dblBankSeniorRecoveryRate,
			...
			if (0 != i) {
				aBKV1[i] = BurgardKjaerBuilder.HedgeErrorDualBond (
					adtVertex[i],
				...
				aBKV2[i] = BurgardKjaerBuilder.HedgeErrorDualBond (
					adtVertex[i],
