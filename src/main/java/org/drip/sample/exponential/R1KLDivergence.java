...
/**
 * <i>R1KLDivergence</i> illustrates the Kullback-Leibler Divergence generated from a pair of R<sup>1</sup>
...
public class R1KLDivergence
{
		...
		throws Exception
	{
		...
		double[] lambda1Array = {0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1., 2., 3., 4., 5.};
		double[] lambda2Array = {0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1., 2., 3., 4., 5.};
		...
		System.out.println (
			"\t||      - Kullback Leibler Divergence Metric (Right Columns)"
		);
		...
		for (int i = 0; i < lambda1Array.length; ++i)
		{
			String metricDisplay = "\t|| {" + FormatUtil.FormatDouble (
				lambda1Array[i], 1, 2, 1.
			) + " , ";

			R1RateDistribution exponentialDistribution1 = new R1RateDistribution (lambda1Array[i]);

			for (int j = 0; j < lambda2Array.length; ++j)
			{
				if (0 == j)
				{
					metricDisplay += FormatUtil.FormatDouble (
						lambda2Array[j], 1, 2, 1.
					) + "} =>";
				}

				metricDisplay += FormatUtil.FormatDouble (
					exponentialDistribution1.kullbackLeiblerDivergence (
						new R1RateDistribution (lambda2Array[j])
					), 3, 1, 1.
				) + " |";
			}
