...
/**
 * <i>R1DensityAndCumulative</i> illustrates the Density and CDF Metrics Suite generated from R<sup>1</sup>
 * 	Exponential Distribution. The References are:
 * 
 * <br><br>
 * 	<ul>
 * 		<li>
...
public class R1DensityAndCumulative
{
		...
		throws Exception
	{
		...
		for (int i = 0; i < lambdaArray.length; ++i)
		{
			...
			for (int j = 0; j < horizonArray.length; ++j)
			{
				metricDisplay += " " + FormatUtil.FormatDouble (
					exponentialDistribution.density (horizonArray[j]), 1, 6, 1.
				) + " |";
			}
		...
		System.out.println (
			"\t||      - Cumulative Probability across Time Horizons (Right-most columns)"
		);
		...
		for (int i = 0; i < lambdaArray.length; ++i)
		{
			...
			{
				metricDisplay += " " + FormatUtil.FormatDouble (
					exponentialDistribution.cumulative (horizonArray[j]), 1, 6, 1.
				) + " |";
			}
