...
/**
 * <i>R1CVaR</i> displays the Generation of the Conditional Value At Risk for the R<sup>1</sup> Exponential
...
public class R1CVaR
{
		...
		throws Exception
	{
		...
		double[] pArray = {0.02, 0.14, 0.26, 0.38, 0.50, 0.62, 0.74, 0.86, 0.98};
		...
		for (int i = 0; i < lambdaArray.length; ++i)
		{
			...
			for (int j = 0; j < pArray.length; ++j)
			{
				display += FormatUtil.FormatDouble (
					exponentialDistribution.cvar (pArray[j]), 3, 3, 1.
				) + " |";
			}
