...
/**
 * <i>R1OrderStatisticsJointMoment</i> demonstrates the Calculation of the Joint First Moment for the
 *  Increasing Order Statistics among a pair of R<sup>1</sup> Variates from a Set Following the Exponential
...
public class R1OrderStatisticsJointMoment
{
		...
		throws Exception
	{
		...
		for (double lambda : lambdaArray)
		{
			...
			R1RateDistribution r1RateDistribution = new R1RateDistribution (lambda);

			for (int orderStatistic1 = 1; orderStatistic1 <= variateCount; ++orderStatistic1)
			{
				for (int orderStatistic2 = 1; orderStatistic2 <= variateCount; ++orderStatistic2)
				{
					if (orderStatistic1 != orderStatistic2)
					{
						System.out.println (
							"\t|| [Lambda: " + FormatUtil.FormatDouble (lambda, 1, 2, 1.) +
