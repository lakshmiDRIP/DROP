...
/**
 * <i>R1DensityAndCumulative</i> illustrates the Buffered Probability Of Exceedance generated from
 * 	R<sup>1</sup> Exponential Distribution. The References are:
 * 
 * <br><br>
 * 	<ul>
 * 		<li>
...
public class R1BPoE
{
		...
		throws Exception
	{
		...
		System.out.println (
			"\t||                            Buffered Probability of Exceedance across Horizon                           ||"
		);
		...
		System.out.println (
			"\t||      - Buffered Probability of Exceedance across Time Horizons (Right-most columns)"
		);
		...
		for (int i = 0; i < lambdaArray.length; ++i)
		{
			...
			R1RateDistribution exponentialDistribution = new R1RateDistribution (lambdaArray[i]);

			for (int j = 0; j < horizonArray.length; ++j)
			{
				metricDisplay += " " + FormatUtil.FormatDouble (
					exponentialDistribution.bPOE (horizonArray[j]), 1, 6, 1.
				) + " |";
			}
