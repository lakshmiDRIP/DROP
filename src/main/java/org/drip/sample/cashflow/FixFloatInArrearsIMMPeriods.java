...
/**
 * <i>FixFloatInArrearsIMMPeriods</i> demonstrates the Cash Flow Period Details for an In-Arrears Fix-Float
 * IMM Swap.
 *  
 * <br><br>
 *  <ul>
...
public class FixFloatInArrearsIMMPeriods {
		...
		throws Exception
	{
		...
		FixFloatComponent ffcInArrearsIMM = FixFloatInArrears (
			dtSpot,
			strCurrency,
			"8Y"
		);
		...
		for (CompositePeriod p : ffcInArrearsIMM.referenceStream().cashFlowPeriod()) {
		...
		for (CompositePeriod p : ffcInArrearsIMM.derivedStream().cashFlowPeriod()) {
			int iEndDate = p.endDate();
		...
		for (CompositePeriod p : ffcInArrearsIMM.couponPeriods()) {
