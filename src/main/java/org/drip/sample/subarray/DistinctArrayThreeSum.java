...
import org.drip.graph.subarray.ThreeSumVariantBuilder;
...
/**
 * <i>DistinctArrayThreeSum</i> illustrates the Check that indicates if the Set of Numbers contains 3 that
 * 	Sum to Zero over 3 Distinct Input Arrays. The References are:
 * 
 * <br><br>
 *  <ul>
 *  	<li>
...
public class DistinctArrayThreeSum
{
		...
		throws Exception
	{
		...
		double[] numberArrayA = new double[]
		{
			  4,
			 10,
			  2,
		};
		double[] numberArrayB = new double[]
		{
			-25,
			-10,
			 -3,
		};
		double[] numberArrayC = new double[]
		{
			 -7,
			  8,
			  4,
		...
		for (double number : numberArrayA)
		{
			array = array + number + ", ";
		}
		...
		for (double number : numberArrayB)
		{
			array = array + number + ", ";
		}
		...
		for (double number : numberArrayC)
		{
			array = array + number + ", ";
		}
		...
		System.out.println (
			"\t| QUADRATIC Hash-Table: Zero 3 SUM exists across Arrays {" + array + "} => " +
			ThreeSumVariantBuilder.ThreeDistinctArrays (
				numberArrayA,
				numberArrayB,
				numberArrayC,
				ThreeSumVariantBuilder.HASH_TABLE
			).zeroSumExists()
		);

		System.out.println (
			"\t| QUADRATIC Comparator: Zero 3 SUM exists across Arrays {" + array + "} => " +
			ThreeSumVariantBuilder.ThreeDistinctArrays (
				numberArrayA,
				numberArrayB,
				numberArrayC,
				ThreeSumVariantBuilder.COMPARATOR
			).zeroSumExists()
		);
