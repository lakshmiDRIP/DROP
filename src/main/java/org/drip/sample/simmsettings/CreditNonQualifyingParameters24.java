...
/**
 * <i>CreditNonQualifyingParameters24</i> demonstrates the Extraction and Display of ISDA SIMM 2.4
 * 	Single/Cross Currency Credit Non-Qualifying Bucket Risk Weights, Systemics, and Correlations. The
 * 	References are:
 * 
 * <br><br>
 *  <ul>
...
public class CreditNonQualifyingParameters24
{

	private static final void DisplayRiskWeights()
	{
		Set<Integer> bucketIndexSet = CRNQSettingsContainer24.BucketSet();

		System.out.println
			("\t||-------------------------------------------------------------------------------------------------------------||");

		System.out.println
			("\t||                               2.4 CREDIT NON QUALIFYING BUCKETS RISK WEIGHT                                 ||");
		...
		{
			CRBucket creditQualifyingBucket = CRNQSettingsContainer24.Bucket (bucketIndex);
	...
	private static final void CreditNonQualifyingSystemics()
	{
		...
		System.out.println (
			...
			FormatUtil.FormatDouble (
				CRNQSystemics24.VEGA_RISK_WEIGHT, 3, 2, 1.
			) + " ||"
		);
		...
		System.out.println (
			...
			FormatUtil.FormatDouble (
				CRNQBucketCorrelation24.GT_80PC_OVERLAP_NON_RESIDUAL, 3, 2, 1.
			) + " ||"
		);
		...
		System.out.println (
			"\t|| Non-Residual Correlation <80% Names Overlap         => " +
			FormatUtil.FormatDouble (
				CRNQBucketCorrelation24.LT_80PC_OVERLAP_NON_RESIDUAL, 3, 2, 1.
			) + " ||"
		);
		...
		System.out.println (
			...
			FormatUtil.FormatDouble (
				CRNQBucketCorrelation24.GT_80PC_OVERLAP_RESIDUAL, 3, 2, 1.
			) + " ||"
		);
		...
		System.out.println (
			"\t|| Residual Correlation <80% Names Overlap             => " +
			FormatUtil.FormatDouble (
				CRNQBucketCorrelation24.LT_80PC_OVERLAP_RESIDUAL, 3, 2, 1.
			) + " ||"
		);
