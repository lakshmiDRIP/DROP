...
import org.drip.spaces.big.KNearestPostOffice;
...
public class KNearestServiceLocater
{
		...
		throws Exception
	{
		...
		List<int[]> postOfficeLocationList = new ArrayList<int[]>();

		postOfficeLocationList.add (
			new int[] {-16, 5}
		);
		...
		int[][] closest = new KNearestPostOffice (
		...
		for (int sortedPostOfficeIndex = 0;
			sortedPostOfficeIndex < k;
			++sortedPostOfficeIndex)
		{
			int[] postOfficeLocation = closest[sortedPostOfficeIndex];

			System.out.println ("\t[" + postOfficeLocation[0] + ", " + postOfficeLocation[1] + "]");
		}
