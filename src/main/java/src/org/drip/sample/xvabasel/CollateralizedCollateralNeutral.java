...
/**
 * <i>CollateralizedCollateralNeutral</i> examines the Basel BCBS 2012 OTC Accounting Impact to a Portfolio
...
public class CollateralizedCollateralNeutral {
		...
		throws Exception
	{
		...
		for (int i = 0; i < iNumPath; ++i) {
			...
			AlbaneseAndersen[] aHGVR1 = new AlbaneseAndersen[iNumStep + 1];
			AlbaneseAndersen[] aHGVR2 = new AlbaneseAndersen[iNumStep + 1];
			...
			for (int j = 0; j <= iNumStep; ++j) {
				...
				if (0 != j) {
					CollateralAmountEstimator hea1 = new CollateralAmountEstimator (
					...
					dblCollateralBalance1 = hea1.postingRequirement (dtEnd);

					CollateralAmountEstimator hea2 = new CollateralAmountEstimator (
						positionGroupSpecification,
						new BrokenDateInterpolatorLinearT (
							dtStart.julian(),
							dtEnd.julian(),
