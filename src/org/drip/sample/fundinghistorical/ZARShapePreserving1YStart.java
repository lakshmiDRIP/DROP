...
/**
 * ZARShapePreserving1YStart Generates the Historical ZAR Shape Preserving Funding Curve Native Compounded
 *  Forward Rate starting at 1Y Tenor.
 * 
 * @author Lakshmi Krishnamurthy
 */

public class ZARShapePreserving1YStart {
		...
		throws Exception
	{
		...
		for (int i = 0; i < iNumClose; ++i) {
			...
			for (String strInTenor : astrInTenor) {
				for (String strForTenor : astrForTenor) {
					double dblNativeForwardRate = Double.NaN;

					try {
						dblNativeForwardRate = fcm.nativeForwardRate (
							strInTenor,
							strForTenor
						);
					} catch (Exception e) {
					...
					strDump += "," + FormatUtil.FormatDouble (dblNativeForwardRate, 1, 5, 100.);
				}
			}
