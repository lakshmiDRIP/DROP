...
public class CMVReconciler2 {
		...
		throws Exception
	{
		...
		System.out.println (
			...
			FormatUtil.FormatDouble (pfOptimal.optimalPortfolio().notional(), 1, 2, 1.) + "   ||"
		);

		System.out.println (
			"\t| Optimal Portfolio Input Return       : " +
			FormatUtil.FormatDouble (dblPortfolioDesignReturn, 1, 2, 100.) + "%  ||"
		);

		System.out.println (
			"\t| Optimal Portfolio Expected Return    : " +
			FormatUtil.FormatDouble (pfOptimal.optimalMetrics().excessReturnsMean(), 1, 2, 100.) + "%  ||"
		);

		System.out.println (
			"\t| Optimal Portfolio Standard Deviation : " +
			FormatUtil.FormatDouble (pfOptimal.optimalMetrics().excessReturnsStandardDeviation(), 2, 2, 100.) + "% ||"
		);

		System.out.println (
			"\t| Excel Portfolio Standard Deviation   : " +
			FormatUtil.FormatDouble (Math.sqrt (pfReconciler.variance (ausp)), 2, 2, 100.) + "% ||"
		);
