...
public class Table4Reconciler {
		...
		throws Exception
	{
		...
		R1MultivariateNormal viewDistribution = R1MultivariateNormal.Standard (
			new MultivariateMeta (new String[] {"TABLE 4"}),
		...
		double[] adblBayesPELoadingsReconciler = new double[] {
			0.302
		};
		...
		System.out.println ("\t| DELTA => " + FormatUtil.FormatDouble (dblRiskFreeRate, 1, 8, 1.) + "   ||");
		...
		for (int i = 0; i < adblInterViewComponent.length; ++i)
			System.out.println (
				...
				FormatUtil.FormatDouble (adblBayesPELoadingsReconciler[i], 1, 3, 1.) + " ||"
			);
